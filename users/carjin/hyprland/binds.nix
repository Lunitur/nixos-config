{ osConfig, lib, ... }:
{
  wayland.windowManager.hyprland.settings = {
    bind = [
      ...
      "SUPER,       Z, exec, cliphist list | fuzzel -d | cliphist decode | wl-copy"
      ...
      "SUPER,       B, exec, nu -c 'cd ~/Nextcloud; ls skripte/**/* | append (ls books/**/*) | append (ls ostalo) | append (ls cheatsheets) | where type == file | $in.name | str join \"\\n\" | fuzzel -d | if ($in | is-not-empty) {zathura $in}'"
      "SUPER,       C, exec, nu -c 'cd ~/Nextcloud/cheatsheets; ls | $in.name | str join \"\\n\" | fuzzel -d | if ($in | is-not-empty) {zathura $in}'"
