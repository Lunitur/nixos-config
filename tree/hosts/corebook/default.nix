{ config, ... }: {
  flake.modules.nixos.hosts.corebook.default = { config, lib, pkgs, pkgs-unstable, ... }:
  {
    imports = [
      config.flake.modules.nixos.hosts.corebook.hardware
      config.flake.modules.nixos.hosts.corebook.monitor
