{ config, inputs, ... }: {
  flake.modules.nixos.hosts.nano.default = { config, pkgs, ... }: {
    imports = [
      inputs.simple-nixos-mailserver.nixosModule
      config.flake.modules.nixos.hosts.nano.hardware
      config.flake.modules.nixos.hosts.nano.arhivar
      config.flake.modules.nixos.network.headscale
      ../../../../users/carjin/user.nix
    ];

    # From common.nix
    nix.settings.trusted-users = [ "lsimek" ];
